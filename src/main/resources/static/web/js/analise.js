//-------ROTA--------
Path.map("#/listaAnalise").to(openListaAnalise);
Path.map("#/analise/:id/:nome").to(function() {
	var id = this.params['id'];
	var nome = this.params['nome'];
	console.log(id, nome);
	var url = "./app/analise/index.html";
...
function infoAnalise() {
	$.get("base/analise/" + idAnalise + "/index.html", function(data) {
		$('#myModal .modal-title').html("Objetivo da analise");
...
function entraModoEdicao() {
	...
	//openDado(idAnalise, modoEdicao);
	showAnalise(idAnalise);
}
function showAnalise(id, nome) {
	if( nome ){
		$("#tituloPagina").html(
			"<h3><a href='#' onClick=\"infoAnalise(); return false;\">" + nome
					+ "</a></h3>");
	}
...
function montaAnalise(json) {
	...
	$("#tituloPagina").html(
			"<h3><a href='#' onClick=\"infoAnalise(); return false;\">" + json.nome
			+ "</a></h3>");
...
function propostaEscolhida(me) {
	console.log('>>>propostaEscolhidax<<<');
	var parent = me.parentElement;
	var nome = parent.innerText;
	console.log('-->' + nome);
	...
	if ($(me).is(':checked')) {
		...
		showPropostaAnalise($(me).attr("name"), p, nome);
...
function novaResposta(me, resposta) {
	...
	/*
	 * if( nodes.length < 3){ div.className='panel-body'; }
	 */
	...
	var html = '\
			...
			+ '</span>\
                       ...
                       <button class="btn btn-default proposta" type="button" onClick="kb( this )"><i class="fa fa-life-ring"></i></button>\
	...
	if (!modoEdicao && !isAtributoModeEdicao) {
		...
		if (propostas) {
			...
			var i = 0;
			for (i = 0; i < propostas.length; i++) {
				var pro = propostas[i].split("#");
				showPropostaAnalise(pro[0], div, pro[1]);
			}
		}
	}
...
function editarOuSalvar(me) {
	...
	if (me.firstChild.className == "fa fa-save") {
		...
		if (isAtributoModeEdicao == true && id != 'q' && id != 'r') {
			...
			json["tipo"] = "nome";
			console.log(json);
			$.post("/analise/savaAtributo", json).done(function(json) {
				console.log(json);
			});
		}
	} else {
...
function montaListaAnalise() {
	wrapper.getAnalises(function(data) {
		...
		$.each(data, function(index, analise) {
			// var html = "<a href='#' onClick=\"showAnalise(\'"+analise.id+"\',
			// \'"+analise.nome+"\'); return
			// false\"><h4>"+analise.nome+"</h4></a>";
			var html = "<a href='#/analise/" + analise.id + "/" + analise.nome
					+ "' ><h4>" + analise.nome + "</h4></a>";
			console.log('===>' + html);
